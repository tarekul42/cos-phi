...
test('spherical: a single octant triangle has area pi/2', () => {
  const tri = [[0, 0], [90, 0], [0, 90]];
  assert.ok(Math.abs(Math.abs(sphericalPolygonArea(tri)) - Math.PI / 2) < 1e-12);
});

test('spherical: the equator ring bounds a hemisphere (2*pi)', () => {
  const equator = [[0, 0], [90, 0], [180, 0], [-90, 0]];
  assert.ok(Math.abs(Math.abs(sphericalPolygonArea(equator)) - 2 * Math.PI) < 1e-10);
});
...
test('densifying a ring does not change its spherical area', () => {
  const ring = [[0, 0], [40, 10], [30, 55], [-25, 40], [-30, -5]];
...
test('projectRing handles an antimeridian-crossing ring without smearing', () => {
  ...
  const ring = [[179, 10], [-179, 10], [-179, 12], [179, 12]];
...
test('projected area / spherical area == 1 for all 177 countries', () => {
  ...
  console.log('  worst ratios:', rows.slice(0, 5)
    .map((r) => `${r.name}=${r.ratio.toFixed(6)}`).join(', '));

  assert.ok(dev < 0.002, `worst: ${worst.name} ratio ${worst.ratio} (dev ${(dev * 100).toFixed(3)}%)`);
});
...
test('absolute areas match published country areas within 8%', () => {
  ...
  for (const [name, expectedKm2] of Object.entries(known)) {
    ...
    console.log(`  ${name}: got ${Math.round(got).toLocaleString('en-US')} km², ` +
