...
import { partials, omegaDeg, distortionMetrics } from '../src/distortion.js';
...
test('monotonic: Y\'(theta) > 0 on [-pi/3, pi/3] for every family member', () => {
  for (const [, a] of FAMILIES) {
    for (let i = -600; i <= 600; i++) {
      ...
      assert.ok(yCurveDeriv(theta, a) > 0, `Y'(${theta}) = ${yCurveDeriv(theta, a)}`);
    }
  }
});
...
test('height pinned: ours share the published map height Y(pi/3)', () => {
  ...
  assert.deepEqual(PUBLISHED_A, { a1: 1.340264, a2: -0.081106, a3: 0.000893, a4: 0.003796 });
});
...
test('criterion: equator not degraded, aspects as expected', () => {
  ...
  const fixedAspect = bounds(OURS_FIXED_ASPECT_A).width / bounds(OURS_FIXED_ASPECT_A).height;
  assert.ok(freeAspect > 1.9 && freeAspect < 2.2, `free aspect ${freeAspect}`);
  assert.ok(Math.abs(fixedAspect - pubAspect) < 1e-9, `fixed aspect ${fixedAspect} vs ${pubAspect}`);
});
...
test('inverse roundtrip with our coefficients', () => {
  for (const lat of [-73.5, -45, -0.5, 0, 12.3, 48.9, 82]) {
    for (const lon of [-179.9, -60, 0, 60, 179.9]) {
      ...
      assert.ok(Math.abs(back.phi * (180 / Math.PI) - lat) < 1e-9, `lat ${lat}`);
      assert.ok(Math.abs(back.lambda * (180 / Math.PI) - lon) < 1e-9, `lon ${lon}`);
    }
  }
});
