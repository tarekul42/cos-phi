...
test("Jacobian equals cos(phi) everywhere (equal-area proof)", () => {
  ...
  for (let latDeg = -85; latDeg <= 85; latDeg += 5) {
    for (let lonDeg = -180; lonDeg <= 180; lonDeg += 15) {
      ...
      const dxdL =
        (project(phi, lambda + h).x - project(phi, lambda - h).x) / (2 * h);
      const dxdP =
        (project(phi + h, lambda).x - project(phi - h, lambda).x) / (2 * h);
      const dydL =
        (project(phi, lambda + h).y - project(phi, lambda - h).y) / (2 * h);
      const dydP =
        (project(phi + h, lambda).y - project(phi - h, lambda).y) / (2 * h);
  ...
  assert.ok(
    worst < 1e-7,
    `worst relative Jacobian error ${worst} at ${JSON.stringify(worstAt)}`,
  );
});
...
test("poles and equator land where expected", () => {
  ...
  assert.ok(
    north.x === south.x,
    "poles map to a line, same x for both hemispheres",
  );
});
